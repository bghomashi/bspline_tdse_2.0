...
bool ValidateAndRunEigenstates(int argc, char **args, const std::string& filename);

int main(int argc, char **args) {
    if (!ValidateAndRunEigenstates(argc, args,"input.json"))
        return -1;
    return 0;
}
...
bool ValidateAndRunEigenstates(int argc, char **args, const std::string& filename) {
    ...
    CurrentDensity cdensity;
    if (!cdensity.Validate(input))
        return false;
