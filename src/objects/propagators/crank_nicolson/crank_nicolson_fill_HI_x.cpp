...
void CrankNicolson::FillInteractionX(Matrix& HI) {
            ...
            for (int l1 = std::abs(m1); l1 <= lmax; l1++) {
                ...
                if (l1+1 <= lmax && l1+1 >= std::abs(m2)) {
                    ...
                    [=,&ddr,&invR](int row, int col) {
                        int i = row % N, j = col % N;                     // TODE: change i,j to anything else
                        double a = -sqrt((l2+m2) * (l2+m2-1) / (2.*l2 + 1.) / (2.*l2 - 1.));

                        return (ddr(i, j) + double(l2)*invR(i, j))*YlmXYlm(l1,m1,l2,m2);
                    });
                }
                ...
                if (l1-1 >= 0 && l1-1 >= std::abs(m2)) {
                    ...
                    [=,&ddr,&invR](int row, int col) {
                        ...
                        return (ddr(i, j) - double(l2+1)*invR(i, j))*YlmXYlm(l1,m1,l2,m2);
                    });
                }
            }
        }
